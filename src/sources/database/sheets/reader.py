from src.sources.database.sheets.client import GoogleSheetClient
...
class GoogleSheetReader:
    ...
    def read_sheet(self, google_sheet_info: dict):
        ...
        Args:
            google_sheet_info: dict
                sheet_id: str  (toma precedencia sobre sheet_name)
                sheet_name: str
                worksheet: str
        """
        try:
            if "sheet_id" in google_sheet_info:
                sheet = self.client.open_by_key(google_sheet_info["sheet_id"])
            else:
            ...
            identifier = google_sheet_info.get("sheet_id", google_sheet_info.get("sheet_name", "DESCONOCIDO"))
            print(f"Error al leer la hoja: '{identifier}'. Detalle: {e}")
            raise
    ...
    def update_sheet(self, google_sheet_info: dict, clear_data: bool = False):
        ...
        if clear_data:
            worksheet.clear()
            set_with_dataframe(worksheet, df, row=start_row, col=1)
        else:
            set_with_dataframe(worksheet, df, row=start_row, col=1, include_column_header=False)
